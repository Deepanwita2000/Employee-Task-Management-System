function validateUser() {
    ...
    const yoeField = document.getElementById("yop"); 
    const yoe_error = document.getElementById("yop_error"); 
    const yoe = yoeField.value.trim()
    ...
    //age
    if(age === ''){
        age_error.innerHTML = `<span class="text-danger">age field cannot be blank  .</span>`
    }
    else if(Number(age) < 21 || Number(age) > 50){
        age_error.innerHTML = `<span class="text-danger">Age limit exeeded.Age must be within 21-50 yrs.</span>`
    }

    // yoe
    if(yoe === ''){
        yoe_error.innerHTML = `<span class="text-danger">(Exp.) is required.</span>`
    }
    else if( Number(yoe) < 0 || Number(yoe) > 40){
        yoe_error.innerHTML = `<span class="text-danger">invalid data.Must be within 0-40 range.</span>`
    }
