...
from .models import  User, UserToken,OTP
...
#             #    ---------------------------------------------------------------- extra
#             # send otp email
#             mail_subject = 'Please verify with otp'
#             email_template = 'account/email/send_otp.html'
#             otp_code = random.randint(100000,999999)
#             print(f"otp : {otp_code} sent to {user}")
#             send_otp_email(request, user, mail_subject, email_template ,otp_code)
#             print("Verification email sent to:", email)
#             uid = urlsafe_base64_encode(force_bytes(user.pk)),     # Encode the user's primary key
#             token = default_token_generator.make_token(user),      # Generate a token for the user
#             return JsonResponse({
...
#                 'redirect_url':f'/account/sent_otp/{otp_code}/'
...
# def sent_otp(request,otp_code):
#     user=request.user
#     print(f"inside verify_oty : {user} , {user.id}")
#     if request.method == 'GET':
#          return render(request,'account/otpForm.html' , {'otp_code':otp_code})
