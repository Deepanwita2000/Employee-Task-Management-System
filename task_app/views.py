...
def view_team_members(request , proj_id=None):
    ...
    return render(request , 'employee_app/team_members.html' , {'tasks':tasks , 'project':proj})
...
def chatBot(request):
    if request.method == 'POST':
        audio_file_url = None
        print(request.POST , type(request.POST)) # dict
        question = request.POST.get("quest" , "").strip()
        print(question)
        print(type(question))
        groq_api_key = os.getenv('GROQ_API_KEY')
        llm = ChatGroq(
        model="llama-3.3-70b-versatile",
        temperature=0,  # lie between 0 to 1
        ...
        return JsonResponse({"answere":formatted_response})
